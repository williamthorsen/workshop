...
- `vitest.config.ts`, `vitest.root.config.ts`, and `vitest.shared.ts`: Vitest projects for packages and for root-level tests respectively, and the layer that every config in the repo passes to the factory; the root variant excludes every workspace package. `nmr test:watch` resolves `vitest.config.ts` and collects the whole tree through it, loading no package config. The root suite's `vitest-shared-options` test fails a config that omits the layer.
