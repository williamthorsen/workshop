/** Symbols imported by a kit through one specifier that the runner's matching entry point does not export. */
...
export interface UnresolvableImports {
  /** `readyup` subpaths that the runner does not publish, so nothing they name can resolve. */
...
/**
 * A compiled kit binding readyup symbols that the running readyup cannot supply.
 *
...
export class UnresolvableKitImportsError extends Error {
  ...
  constructor(findings: UnresolvableImports) {
    super('Kit imports readyup symbols that the runner does not export');
