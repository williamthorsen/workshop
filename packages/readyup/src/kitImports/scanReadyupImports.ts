 ...
 * deciding whether the text is an import at all. A default binding sits outside the braces and is skipped: jiti's CJS
 * interop supplies a module object for it, so a runner exporting no `default` breaks nothing.
 */
function readBoundNames(statement: string): string[] {
  ...
  for (const entry of braced.split(',')) {
    const name = LEADING_NAME_PATTERN.exec(entry)?.[1];
