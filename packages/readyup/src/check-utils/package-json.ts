 ...
 * version that resolves. A `catalog:` specifier is resolved through the `pnpm-workspace.yaml` in the working
 * directory and measured against the version found there; one that resolves to no version meets no floor. `exempt`
 * receives the specifier as declared, so a catalogued dependency is passed to it as `catalog:`; it adds further
 * exemptions and cannot remove the `workspace:` one.
 *
