 ...
 * the file's last line where it is the last, because a generic constraint's braces and a return-type
 * annotation's both defeat a scan for the closing brace and a span cut short reports the implementation the rule
 * exists to exempt. A re-exporting barrel declares no implementation and holds no exempted lines, a file
 * declaring the name without exporting it is a hand-roll and is still reported, and a file declaring the export
 * under another name and renaming it on export from a second file is not recognized.
 */
