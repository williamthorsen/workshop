 ...
 * which is what lets `list` fall back to enumerating the same files that `run` would load when no manifest
 * sits beside them, and readyup's own `packaging` check holds a published package to it. A project's own
 ...
 * otherwise.
 *
 * Falls back to the convention directory where no config was loaded, which is the path that an external
 * source flag takes.
 */
