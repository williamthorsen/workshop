...
/**
 * Returns the name of the kit that a file contains, which is its path relative to the directory that roots it
 * with `extension` removed.
 *
 * Separators become `/` on every platform, because the name is what the manifest records and what
 * `rdy run` takes: a name containing a backslash would name one file on Windows and another nowhere else.
...
/**
 * Returns the name of the kit that a bundle contains, given the output directory against which names are resolved.
 *
 * A bundle outside that directory takes its own basename. `rdy run` locates a kit by joining its name onto the
 * output directory, so no name resolves to such a bundle, and the basename is the name that its reader recognizes.
 */
