...
const KIT_SOURCE = [
  ...
  'export const kit = { dependencyData, helper, legacy, meta, pkg, settings, shared, tiny };',
].join('\n');

const HELPER_SOURCE = `import shared from './shared.json' with { type: 'json' };\nexport const helper = shared;\n`;
...
describe('buildBundle inlined JSON', () => {
  ...
  beforeAll(async () => {
    ...
    for (const fileName of ['legacy.json', 'picked.json', 'settings.json', 'shared.json']) {
