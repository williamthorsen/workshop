 ...
 *
 * The one implementation of the projection on which every reader of a recorded input relies. Once a
 ...
 *
 * Returns the serialized form rather than the projected object, to let a compile hash exactly what it
 * substitutes.
 *
 * Throws when the file is unreadable, contains invalid JSON, contains something other than an object, or no longer
 * contains a path named by the specifier. `describeJsonProjectionFailure` words any of those four for a report.
 */
