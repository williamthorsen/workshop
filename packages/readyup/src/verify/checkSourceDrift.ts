 ...
 *
 * `unverified` when the entry records no source or no hash, which means it predates the feature or
 * was written with `--skip-manifest`; `missing` when the recorded source is gone; `stale` when the
 * hashes differ; and `ok` when they match. This is orthogonal to the target verdict: A kit can be
