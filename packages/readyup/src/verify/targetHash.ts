...
/**
 * Returns the hash of a buffer as the manifest records it.
 *
 * This and its two siblings write a record; they are not how one is read back. A reader compares
 * through `hashToRecordedLength`, which honors whatever prefix was used by the compile that wrote
 * the record, so hashing here and comparing the result would reinstate a fixed-length comparison.
 */
export function hashBytes(bytes: Buffer): string {
  return createHash('sha256').update(bytes).digest('hex').slice(0, HASH_PREFIX_LENGTH);
}

/** Returns the hash of a file as the manifest records it. */
