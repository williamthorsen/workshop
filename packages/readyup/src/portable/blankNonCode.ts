...
// whole and a postfix one is therefore never checked against this set.
 ...
 * around the prose is not. The output matches the input in length and in every newline position, which lets a
 * caller take an offset from one text and read the other at it: `getLineAtOffset` resolves an offset
 ...
 *
 * When a `/` could open a regular expression or divide, the ambiguity resolves toward division, and a quoted
 ...
 * operand rather than replacing it, which is `++`, `--`, and TypeScript's `!`. A property spelled like a keyword is
 * read as the property that it is, because a member name keeps the `.` or `#` that introduced it. A construct
 ...
 *
 * Reads JavaScript-family syntax. A source in another language yields arbitrary output rather than an error. A
 * filter selecting `.md` or `.yaml` paths should not call it.
 */
...
/** Runs one pass over a source, blanking comments always and literals when the mode requires it. */
...
/** Blanks a literal's text span when the scan blanks literals, and returns the span's end either way. */
function blankLiteralText(scan: Scan, from: number, to: number): number {
  return scan.blanksLiterals ? blankSpan(scan, from, to) : to;
}

/**
 * Blanks a quoted string's text and returns the offset past its closing delimiter, or past the opening one when
 * the line ends first.
 */
...
/** Returns the offset past a block comment's terminator, or the source's end if it never closes. */
function findBlockCommentEnd(source: string, from: number): number {
  const end = source.indexOf('*/', from + 2);
  return end === -1 ? source.length : end + 2;
}

/** Returns the offset past the comment opening at an offset, or `undefined` if none opens there. */
...
/** Returns the offset of the line break ending the line that contains an offset, or the source's end. */
function findLineEnd(source: string, from: number): number {
  const end = source.indexOf('\n', from);
  return end === -1 ? source.length : end;
}

/** Returns the offset past a regular expression's closing delimiter, or `undefined` if the line ends first. */
 ...
 * `previousToken` unchanged keeps that decision with the operand. `startsRegex` is the test because a
...
/**
 * Returns the token that a word forms, keeping the punctuation that introduced it when the word is a member name.
 *
 * A `/` after a property or private-field name divides, because the name completes an operand. A name stored bare
 * would be looked up as the keyword that it spells and open a regular expression instead, because every member of
 * `EXPRESSION_KEYWORDS` is also a legal member name: `ops.delete`, `bytes.in`, `this.#delete`.
 */
...
function scanCode(scan: Scan, from: number, isInterpolation: boolean): number {
  const { source } = scan;
  // The token against which a `/` is classified: the operand before it when one is complete. A member name takes
  // its introducing `.` or `#`, a postfix `++` or `--` is one token, and a postfix `!` leaves this untouched.
  let previousToken = '';
  let braceDepth = 0;
  let index = from;
