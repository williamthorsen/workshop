...
/** The strings that a pragma may write to name one check, and the one printed beside its findings. */
 ...
 *
 * A kit published by a package namespaces its checks under that package's name with the scope stripped, and
 * accepts the fully-qualified name too. The bare id is not accepted there: The namespace keeps two kits'
 * same-named checks apart, and accepting the bare form would give that away. Every other provenance, and a
 * kit reached with none, leaves the bare id standing, so the id printed beside a finding is always the id
 * written by a pragma.
 */
