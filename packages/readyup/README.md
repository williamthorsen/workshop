...
Unlike every other listing, this view carries no heading rules. The two levels it nests would be a stroke apart, and their glyphs already tell them apart; under `--style plain`, where the role glyphs are empty, the indentation carries all three levels on its own. That is also why each command is labelled `To run:`: it shares a column with the kits beneath it, and the label is what keeps it from reading as one more kit.
