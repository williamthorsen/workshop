...
- 🚨 **Breaking:** Name kits compiled from nested source directories, and enumerate them (#483)

  - Changes the name of a kit compiled from a subdirectory of `compile.srcDir` to its path below that directory, so a kit compiled from `ops/deploy.ts` is named `ops/deploy` and runs under that name, while a kit at the top of `compile.srcDir` keeps the name that it has.
  - Extends bundle enumeration to the whole kit directory, so `rdy list`, `rdy run --all`, `rdy list --from <dir>`, and the kits of a configured package include nested kits when no manifest records them.
  - Allows `rdy compile <file>` under `--config`, naming that kit from the named config's `compile.outDir` so that it matches the entry that a sweep of the same source records.

  Migration: Rename each nested kit to its full path wherever a script, a config, or a manifest reader names it, so that a kit compiled from `ops/deploy.ts` is `ops/deploy` rather than `deploy`. A nested kit could not be run by name before this change, so only `--all` runs and manifest readers observed the old names.

### 🐛 Bug fixes

- Prevent a value preview from ending in a lone surrogate (#479)

  - Fixes a value preview in a `rdy` check diagnostic that truncation cut into an emoji, an accented character, or another cluster stored as more than one UTF-16 code unit; a split emoji rendered as a replacement character.

- 🚨 **Breaking:** Resolve a project's kits against its configured directories (#485)

  - Fixes `rdy run <kit>` and `rdy run --jit <kit>`, which looked only in the hardcoded `.readyup/kits` and so found no kit in a project whose `compile.outDir` or `compile.srcDir` names another directory.
  - Corrects `rdy run --all --jit`, which read every `.ts` file at the top level of the kits directory rather than the sources that `compile.include` and `compile.exclude` select, skipping a kit compiled from a nested source and running a shared helper module as a kit.
  - Stops a kit name that is absolute or that holds a `..` segment from loading a module outside the kits tree, as `rdy run ../../../../../../etc/hosts` did: `rdy run` now exits with a usage error naming `--file` and `--from dir:` as the ways to run a kit from elsewhere.
  - Splits `rdy list` into a "Sources" section and an "Internal" section, each headed by the command that runs its rows, since a project that set `internal.dir` or `internal.infix` previously got no row for a source that `rdy run --jit` runs.

  Migration: In a project that sets both `internal.dir` and a `compile.srcDir` outside `.readyup/kits`, move the internal kits under `compile.srcDir`, or rewrite `internal.dir` as their path relative to it, because `internal.dir` now resolves against the compile directories rather than against `.readyup/kits`. A project that leaves `compile.srcDir` at its default needs no edit.

### ♻️ Refactoring

- Replace the local style resolver and glyph model with toolbelt.terminal (#486)

  - Replaces readyup's local style resolver (`layout/resolveStyle.ts`) and glyph tables (`layout/richFormatter.ts`, `layout/plainFormatter.ts`) with `resolveOutputStyle` and `defineGlyphSet` from `@williamthorsen/toolbelt.terminal`, defining one glyph set in `layout/formatter.ts` that keeps the existing token names, emoji, and plain words.
  - Changes style selection to the shared resolver's rules: `auto` renders plain under `TERM=linux`, and `rdy` reports a spaced `--style` followed by a dash-led argument as `--style requires a value` rather than as an unknown style.
