...
/**
 * One artifact a claim recovers, with the parts its id was composed from.
 *
 * The kind and the slug ride along rather than being split back out of the id, which the contract calls opaque. They
 * are what lets a consumer describe an artifact deleted from every source, the case a catalog can no longer describe
 * and the one this whole scan exists to reach.
 */
 ...
 *
 * `claims` contains more than one where two deployments' claim rules both match the path, which two kinds rooted at one
 ...
 *
 * `kindId` names the deployment this belongs to, because two kinds may route into one host and each declares its own
 * markers.
 */
export type HostState =
  ...
  | {
      ...
      /** Empty unless the host has one well-formed region: nothing may be read out of a damaged one. */
 ...
 *
 * Keyed by path rather than by kind: an entries host belongs to no artifact kind, and several declarations may own
 ...
 *
 * A path is claimed once however many deployments match it, keeping each artifact they recover, so a consumer reading
 * the result as the destination's own tree finds every path in it exactly where a destination holds it.
 *
 * Three exclusions bound the claim, and each closes off a way the rule could take a file that is not the engine's.
 * Names beginning with a dot or an underscore are support content, the rule a source is enumerated by. Within a claimed
 * artifact, only tool state is passed over, the rule that artifact's digest is taken by, so what a source ships and
 * what a destination claims agree. Every declared host is excluded wherever it falls, region and entries alike, since a
 * host is a file the engine does not otherwise own and a tree claim over one would offer it up for deletion.
 *
 ...
 * layout contributes everything beneath it, an artifact deploying its assets beside the file containing its
 * frontmatter.
 */
...
/** Reads one region host, along with the contributions its region contains. */
