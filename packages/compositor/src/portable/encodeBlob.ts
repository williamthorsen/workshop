...
/**
 * Encodes bytes as the blob a plan carries, as text where the bytes are text and byte for byte where they are not.
 *
 * The choice is made by re-encoding the decoded text and comparing: bytes that survive the round trip are text, and
 * bytes that do not would lose their identity in a plan that carried them as a string. Deciding this way is what keeps
