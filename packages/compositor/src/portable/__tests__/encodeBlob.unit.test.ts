...
describe(encodeBlob, () => {
  it('stores text as text, so a plan a human reads shows the body', () => {
    expect(encodeBlob(Buffer.from('# Review\n', 'utf8'))).toStrictEqual({ encoding: 'utf8', data: '# Review\n' });
  });

  it('stores text outside the ASCII range as text', () => {
  ...
  it('stores bytes no UTF-8 reading survives as base64', () => {
