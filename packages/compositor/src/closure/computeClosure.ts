 ...
 *
 * The document contains only what a reader of it needs, so a kind is recorded without the on-disk layout resolution
 * used and a source without the directory it resolved to. Neither means anything to a reader of a closure, and a
 * filesystem path has no business travelling inside a payload.
 */
