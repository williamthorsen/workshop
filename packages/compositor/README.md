...
The destination collision is the one diagnostic shape validation adds. Composing finds the same collision from the entries two deployments both drafted, which takes target state to reach; here it follows from deployed paths alone, so it is answerable before any destination has been looked at.
