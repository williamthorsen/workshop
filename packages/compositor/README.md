...
Neither form's declaration is trusted on faith. `assertRenderTargetsAreConsistent` reports markers that could not delimit a span, a contribution template that does not name its contributor exactly once, and a host standing where a layout root's directory goes -- before any of them could surface at whichever file first reached the fault. A host whose _shape_ a layout's claim rule could match is a different question, decidable only against the slugs a catalog turns out to carry, and it belongs to the scan that reads current target state rather than here.
